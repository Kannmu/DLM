...
To support the absolute detection-threshold measurements in Experiment~2, the device output had to be adjustable in physically meaningful and approximately equal perceptual increments. However, the firmware controls the ultrasonic emission strength through pulse-width modulation of the bipolar driver stage rather than through a directly linear acoustic amplitude source. Consequently, the user-level strength parameter is not linearly proportional to the emitted acoustic pressure or to the resulting acoustic radiation force on the skin. Because the adaptive staircase procedure assumes a monotonic and well-characterized mapping between command level and physical stimulus magnitude, we explicitly derived and validated the transfer function from the PWM command to the effective acoustic output.

The derivation starts from the bipolar square-wave excitation applied to each transducer at the carrier frequency of \qty{40}{\kilo\hertz}. Let $D$ denote the one-sided PWM duty ratio within the available half-cycle control range, with $D \in [0,0.5]$. In the firmware, the dimensionless strength command $S \in [0,100]$ is mapped linearly to the duty ratio according to
\begin{equation}
    D = 0.5\frac{S}{100}.
\end{equation}
For a bipolar rectangular waveform with duty ratio $D$, Fourier decomposition shows that the amplitude of the fundamental \qty{40}{\kilo\hertz} component is proportional to $\sin(\pi D)$. Since focal acoustic pressure is proportional to the carrier-frequency fundamental voltage component, the normalized acoustic pressure can be written as
\begin{equation}
...
We then performed a dedicated benchtop calibration experiment to test this prediction empirically. The measurement setup is shown in \cref{fig:supplementary_figure_intensity_test}a. A rigid holder was custom-designed and fabricated by 3D printing in PETG so that it mechanically registered to the outer geometry of the phased-array housing. The holder positioned a receiver transducer of the same model as the transmitting elements directly above the geometric center of the array at the nominal focus location $(0,0,\qty{0.1}{\meter})$. This arrangement minimized placement variability across measurements and ensured that all recordings were obtained at a fixed spatial point coincident with the intended focal target used in the psychophysical experiments.

During calibration, the UMH device was configured in a static single-focus mode, and the focal point was kept fixed at the center of the measurement volume. The strength command was swept from $S=5$ to $S=100$ in increments of 5, corresponding to 20 sampled duty ratios from $D=0.025$ to $D=0.5$. At each setting, we recorded the steady-state peak-to-peak voltage $V_{pp}(S)$ generated across the receiver transducer terminals using an oscilloscope (DS1104, RIGOL TECHNOLOGIES CO., LTD) after the field had stabilized. The maximum measured value occurred at full-scale drive, where $V_{pp}(100)=\qty{14.1}{\volt}$. We then formed the measured normalized pressure as
\begin{equation}
...
The measured data closely followed the theoretically predicted sinusoidal transfer law over the full command range, as shown in \cref{fig:supplementary_figure_intensity_test}b. For normalized pressure, the correlation between measurement and theory was $r=0.998$ with a root-mean-square error of $\mathrm{RMSE}=0.031$. For normalized intensity, the agreement remained similarly high, with $r=0.997$ and $\mathrm{RMSE}=0.040$. These values indicate that the hardware reproduces the expected nonlinear control law with high fidelity and that the dominant shaping of output magnitude is captured by the first-principles Fourier model of the PWM-driven bipolar excitation.

The residual structure is also informative. The largest absolute intensity deviation was below 0.07 and occurred near $D=0.20$ (approximately $S=40$), as summarized by the inset histogram in \cref{fig:supplementary_figure_intensity_test}b. The deviations were systematically larger in the lower-duty-ratio regime than near saturation. This pattern is consistent with two practical effects. First, under narrow-pulse excitation the transient electromechanical response of the piezoelectric transducer is more sensitive to finite switching time and internal damping, so the emitted carrier component need not follow the ideal square-wave decomposition exactly. Second, the receiver voltage is smaller at low drive levels, which reduces the measurement signal-to-noise ratio and amplifies the relative contribution of oscilloscope quantization and environmental fluctuations. Even so, the remaining discrepancy was small compared with the dynamic range of the full transfer curve.

This calibration served two purposes for the subsequent psychophysical threshold experiment. On the methodological side, it established a validated forward mapping from the software strength command to physical output, allowing staircase levels to be interpreted in terms of normalized acoustic intensity rather than arbitrary digital units. On the mechanistic side, it confirmed that the device does not introduce a hidden irregularity or non-monotonicity that could confound comparisons among modulation strategies near threshold. Because the experimentally measured transfer function was nearly identical to the theoretical prediction, the command-domain intensity in Experiment~2 could be treated as a well-behaved, monotonic proxy for the actual focal radiation-force magnitude delivered by the UMH hardware.
...
\SupplementarySection{Sensitivity of Spatiotemporal Coherence to Scan Speed (Mach Number) in Unidirectional Lateral Modulation ($ULM_L$)}
