...
class ExperimentApp(ctk.CTk):
    ...
    def setup_connect_tab(self):
        ...
        card_conn = ctk.CTkFrame(container, corner_radius=15, border_width=1, border_color="gray30")
        card_conn.pack(pady=10, padx=20, fill="x", ipadx=20, ipady=20)
        
        ctk.CTkLabel(card_conn, text="Device Connection", font=ctk.CTkFont(size=20, weight="bold")).pack(pady=(5, 15))
        
        # Connection Controls Row
        conn_row = ctk.CTkFrame(card_conn, fg_color="transparent")
        conn_row.pack(fill="x", pady=5)
        
        self.btn_scan = ctk.CTkButton(conn_row, text="↻ Scan Ports", command=self.scan_ports, width=120)
        self.btn_scan.pack(side="left", padx=5)
        
        self.combo_ports = ctk.CTkComboBox(conn_row, values=["No Ports Found"], width=250)
        self.combo_ports.pack(side="left", padx=5, fill="x", expand=True)
        
        self.btn_connect = ctk.CTkButton(card_conn, text="Connect Device", command=self.connect_device, 
                                         height=40, font=ctk.CTkFont(size=15, weight="bold"))
        self.btn_connect.pack(pady=20, fill="x", padx=5)
        
        self.lbl_status = ctk.CTkLabel(card_conn, text="Status: Disconnected", text_color="#FF5555", font=ctk.CTkFont(size=13))
        self.lbl_status.pack(pady=(0, 5))
        
        # --- Demo Mapping Card ---
        card_map = ctk.CTkFrame(container, corner_radius=15, border_width=1, border_color="gray30")
        card_map.pack(pady=20, padx=20, fill="both", expand=True, ipadx=20, ipady=20)
        
        ctk.CTkLabel(card_map, text="Stimulus Mapping (Demo Indices)", font=ctk.CTkFont(size=18, weight="bold")).pack(pady=(5, 10))
        
        self.btn_scan_demos = ctk.CTkButton(card_map, text="Scan Demos (0-10)", command=self.scan_demos, fg_color="gray40", hover_color="gray50")
        self.btn_scan_demos.pack(pady=5)
        
        self.textbox_demos = ctk.CTkTextbox(card_map, height=200, font=ctk.CTkFont(family="Consolas", size=12))
    ...
    def setup_experiment_tab(self):
        ...
        ctk.CTkLabel(frame_left, text="Control Panel", font=ctk.CTkFont(size=22, weight="bold")).pack(pady=(25, 30))
        
        # Participant ID Input
        p_frame = ctk.CTkFrame(frame_left, fg_color="transparent")
        p_frame.pack(fill="x", padx=20)
        ctk.CTkLabel(p_frame, text="Participant ID", font=ctk.CTkFont(size=14), anchor="w").pack(fill="x")
        self.entry_pid = ctk.CTkEntry(p_frame, placeholder_text="e.g. P01", height=35)
        self.entry_pid.pack(fill="x", pady=(5, 15))
        
        self.btn_start_exp = ctk.CTkButton(frame_left, text="Start New Experiment", command=self.start_experiment_flow, 
                                             fg_color="transparent", border_width=2, text_color=("gray10", "#DCE4EE"), 
                                             height=35)
        self.btn_start_exp.pack(pady=5, padx=20, fill="x")

        self.btn_next_block = ctk.CTkButton(frame_left, text="Start Next Block", command=self.next_block_action,
                                            fg_color="#E59400", hover_color="#C27D00", text_color="white",
                                            height=35, state="disabled")
        self.btn_next_block.pack(pady=5, padx=20, fill="x")
        
        # Separator
        ctk.CTkFrame(frame_left, height=2, fg_color=("gray70", "gray30")).pack(fill="x", padx=20, pady=25)
        
        # Block Info
        self.lbl_block_info = ctk.CTkLabel(frame_left, text="Block: - / -", font=ctk.CTkFont(size=16, weight="bold"), text_color="#3B8ED0")
        ...
        self.btn_play_trial = ctk.CTkButton(self.frame_play_container, text="▶ PLAY SEQUENCE", command=self.play_trial_sequence, 
                                            fg_color="#2CC985", hover_color="#229965", text_color="white",
                                            height=60, font=ctk.CTkFont(size=16, weight="bold"))
        self.btn_play_trial.pack(fill="x")
        
        # Progress Bar
        ctk.CTkLabel(frame_left, text="Sequence Progress", font=ctk.CTkFont(size=12)).pack(pady=(20, 5))
        ...
        ctk.CTkLabel(frame_right, text="Participant Response (2AFC)", font=ctk.CTkFont(size=24, weight="bold")).pack(pady=(15, 25))
        ...
        ctk.CTkLabel(self.q1_card, text="1. Intensity", font=ctk.CTkFont(size=16, weight="bold"), text_color="#3B8ED0").pack(anchor="w", padx=10)
        ctk.CTkLabel(self.q1_card, text="Which stimulus felt STRONGER?", font=ctk.CTkFont(size=15)).pack(anchor="w", padx=10, pady=(2, 10))
        
        self.var_intensity = tk.StringVar(value="")
        q1_opts = ctk.CTkFrame(self.q1_card, fg_color="transparent")
        q1_opts.pack(fill="x", padx=20)
        
        self.rb_int_a = ctk.CTkRadioButton(q1_opts, text="Stimulus A (First)", variable=self.var_intensity, value="A", font=ctk.CTkFont(size=14))
        self.rb_int_a.pack(side="left", padx=30, pady=10)
        self.rb_int_b = ctk.CTkRadioButton(q1_opts, text="Stimulus B (Second)", variable=self.var_intensity, value="B", font=ctk.CTkFont(size=14))
        ...
        ctk.CTkLabel(self.q2_card, text="2. Clarity / Size", font=ctk.CTkFont(size=16, weight="bold"), text_color="#3B8ED0").pack(anchor="w", padx=10)
        ctk.CTkLabel(self.q2_card, text="Which stimulus felt SMALLER / CLEARER?", font=ctk.CTkFont(size=15)).pack(anchor="w", padx=10, pady=(2, 10))
        
        self.var_clarity = tk.StringVar(value="")
        q2_opts = ctk.CTkFrame(self.q2_card, fg_color="transparent")
        q2_opts.pack(fill="x", padx=20)
        
        self.rb_clr_a = ctk.CTkRadioButton(q2_opts, text="Stimulus A (First)", variable=self.var_clarity, value="A", font=ctk.CTkFont(size=14))
        self.rb_clr_a.pack(side="left", padx=30, pady=10)
        self.rb_clr_b = ctk.CTkRadioButton(q2_opts, text="Stimulus B (Second)", variable=self.var_clarity, value="B", font=ctk.CTkFont(size=14))
        ...
        # Submit Button
        self.btn_record = ctk.CTkButton(self.frame_record_container, text="Submit Response & Next Trial", command=self.record_response, 
        ...
        # Status Footer
        self.lbl_status_exp = ctk.CTkLabel(frame_right, text="Ready to start experiment", text_color="gray", font=ctk.CTkFont(size=13, slant="italic"))
    ...
    def connect_device(self):
        ...
        try:
            ...
            if self.controller.ping():
                self.lbl_status.configure(text=f"Status: Connected to {port}", text_color="#2CC985") # Green
    ...
    def start_experiment_flow(self):
        ...
        messagebox.showinfo("Info", f"Experiment Started.\nBlock 1: {self.logic.get_current_block_type()}")

    def next_block_action(self):
        if self.logic.next_block():
            self.btn_next_block.configure(state="disabled")
            self.update_ui_for_block()
            messagebox.showinfo("Info", f"Starting Block {self.logic.current_block_index + 1}: {self.logic.get_current_block_type()}")
        else:
    ...
    def update_ui_for_block(self):
        ...
        self.lbl_block_info.configure(text=f"Block: {idx} / {total_blocks} ({block_type})")
        ...
        def run_sequence():
            try:
                ...
                def on_sequence_done():
                    ...
                    self.btn_record.pack(fill="x")
                    self.lbl_status_exp.configure(text="Sequence finished. Please submit response.")
    ...
    def record_response(self):
        ...
        if block_type == "Intensity":
            ...
            if not choice:
                messagebox.showwarning("Warning", "Please select an option for Intensity.")
                return
            ...
            if not choice:
                messagebox.showwarning("Warning", "Please select an option for Clarity.")
                return
            stim_a, stim_b = current
        ...
        try:
            self.logic.save_trial_data(current[0], current[1], chosen_int, chosen_clr, rt)
        except Exception as e:
