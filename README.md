...
考虑到呈现顺序的遗留效应（先出现的刺激会被记忆衰减影响），需要引入正反序与多次重复。每个Block包含30个试次（Trials）（即10个比较对 $\times$ 3次重复）。
2. 硬件与物理环境控制
...
招募至少10名被试进行预实验。实验分为两个独立的Block（模块），分别为“强度比较”模块和“空间精度比较”模块。两个模块的呈现顺序对每个被试进行随机化平衡（例如一半被试先做强度模块，另一半先做精度模块），以消除顺序效应。

- **Block设计与随机化**
  - **Block 1（强度/精度）**：包含30个Trials（10个刺激对组合 $\times$ 3次重复）。
  - **Block 2（精度/强度）**：重新随机化呈现这30个Trials。
  - **双盲控制**：实验者与被试在实验进行中均不知道当前呈现的是哪两个条件。
